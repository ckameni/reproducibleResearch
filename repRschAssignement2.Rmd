...
unique(MergedDf$EVTYPE[grep("heat" ,MergedDf$EVTYPE)])
...
```{r}
# Plotting the 6 events that cause the most injuries
injDamage<-arrange(damageDf[,c(1,3)],desc(SumInjuries)) %>% head(); 

plot_injuries <- function(injuries){
  p <- qplot(data=injDamage,
             x=EVTYPE,
             y=SumInjuries)
  print(p)  
}
arrange(injDamage, SumInjuries) %>% 
  do(plot = plot_injuries(.))
...
# plotting the 6 events  that cause the most fatalies
 fatalDamage<-arrange(damageDf[,c(1,2)],desc(SumFatalities))%>% head()

plot_fatalities <- function(fatalities){
  p <- qplot(data=fatalDamage,
             x=EVTYPE,
             y=SumFatalities)
  print(p)  
}
arrange(fatalDamage, SumFatalities) %>% 
  do(plot = plot_fatalities(.))
...
```{r,injuries, fig.height=4,fig.width=6}

class(injDamage)

hist(injDamage$EVTYPE)
